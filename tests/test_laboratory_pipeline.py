...
def lab_env(tmp_path: Path, monkeypatch):
    ...
    config_dir = tmp_path / "agent_config"; config_dir.mkdir()
    ...
    for agent_id in ["satoshi", "funded-buyer", "steady-eddie-eur", "yolo-sapiens-usd"]:
        (config_dir / f"{agent_id}.json").write_text(json.dumps({
    ...
    (config_dir / "halted-agent.json").write_text(json.dumps({
        "max_order_notional": 1000.0,
        "max_orders_per_day": 5,
        "daily_drawdown_halt_pct": -5.0,
        "allowed_universe": [],
    ...
    (config_dir / "gated-agent.json").write_text(json.dumps({
        "max_order_notional": 1000.0,
        "max_orders_per_day": 5,
        "daily_drawdown_halt_pct": -5.0,
...
class TestLaboratoryPipeline:
    def test_full_pipeline_end_to_end_with_every_rail(self, lab_env, monkeypatch):
        ...
        step_author_orders("funded-buyer", [
            {"action": "BUY", "ticker": "BTC-EUR", "shares": 0.01, "reasoning": "Dip"},
        ], TRADE_DATE, currency="EUR")

        # MAX_ORDER_NOTIONAL: 10 * 700 = 7000 > 1000 cap
        step_author_orders("steady-eddie-eur", [
            {"action": "BUY", "ticker": "MC.PA", "shares": 10, "reasoning": "Luxury overweight"},
        ], TRADE_DATE, currency="EUR")

        # TICKER_NOT_IN_UNIVERSE: MSFT isn't in single-voo
        step_author_orders("gated-agent", [
            {"action": "BUY", "ticker": "MSFT", "shares": 1, "reasoning": "Out of scope"},
        ], TRADE_DATE, currency="EUR")

        # NO_POSITION_TO_SELL: long-only enforcement via rail
        step_author_orders("yolo-sapiens-usd", [
            {"action": "SELL", "ticker": "MSFT", "shares": 1, "reasoning": "Short-attempt guard"},
        ], TRADE_DATE, currency="USD")

        # DAILY_DRAWDOWN_HALT
        step_author_orders("halted-agent", [
            {"action": "BUY", "ticker": "BTC-EUR", "shares": 0.001, "reasoning": "Will halt"},
        ], TRADE_DATE, currency="EUR")
        ...
        with outbox_file.open("a", encoding="utf-8") as f:
            f.write('{"order_id": "malformed_1", "agent_id": "satoshi", "action": "BUY", "shares": 0, "ticker": "BTC-EUR", "currency": "EUR", "ts": "2026-04-17T20:00:00Z", "reasoning": "zero"}\n')
            f.write('{"not valid json\n')  # corrupt

        # NO_PRICE_DATA: author a valid order for a ticker with no store row
        step_author_orders("satoshi", [
            {"action": "BUY", "ticker": "NONEXISTENT-EUR", "shares": 0.01, "reasoning": "No price"},
        ], TRADE_DATE, currency="EUR")
        ...
        step_author_orders("satoshi", [
            {"action": "BUY", "ticker": "MC.PA", "shares": 1, "reasoning": "No money"},
        ], TRADE_DATE, currency="EUR")

        # MAX_ORDERS_PER_DAY: submit 5 (cap) + 1 more for yolo. First we need valid cheap trades.
        for i in range(6):
            step_author_orders("yolo-sapiens-usd", [
                {"action": "BUY", "ticker": "MSFT", "shares": 0.5, "reasoning": f"order {i}"},
            ], TRADE_DATE, currency="USD")
        ...
        (lab_env["config_dir"] / "fail-agent.json").write_text(json.dumps({
            "max_order_notional": 1000.0,
            "max_orders_per_day": 5,
            "daily_drawdown_halt_pct": -5.0,
            "allowed_universe": [],
        ...
        fa_state["positions"] = [{
            "ticker": "BTC-EUR", "shares": 0.0001, "avg_cost": 50000.0,
            "date_opened": "2026-04-16", "grid_level": 0,
        }]
        ...
        step_author_orders("fail-agent", [
            {"action": "SELL", "ticker": "BTC-EUR", "shares": 0.00001, "reasoning": "Race-induced fail"},
        ], TRADE_DATE, currency="EUR")

        # INSUFFICIENT_SHARES: 0.001 > 0.0001 held, notional=50 EUR < 1000 cap → caught before apply.
        step_author_orders("fail-agent", [
            {"action": "SELL", "ticker": "BTC-EUR", "shares": 0.001, "reasoning": "Too many"},
        ], TRADE_DATE, currency="EUR")
        ...
        expected_reasons = {
            "INVALID_SHARES", "MAX_ORDER_NOTIONAL", "MAX_ORDERS_PER_DAY",
            "TICKER_NOT_IN_UNIVERSE", "NO_PRICE_DATA", "INSUFFICIENT_CASH",
            "NO_POSITION_TO_SELL", "INSUFFICIENT_SHARES", "DAILY_DRAWDOWN_HALT",
            "APPLY_TRADE_FAILED",
        }
        ...
        assert set(prompts.keys()) == {"satoshi", "steady-eddie-eur", "yolo-sapiens-usd"}
        ...
        for aid in agent_results:
            mock_resp = json.dumps([{"text": f"{aid} posted.", "mentions": [], "kind": "trade"}])
        ...
        mock_oracle_resp = json.dumps({
            "blog_draft": {"title": "Day 1: Opening", "body_md": "# Day 1", "slug": "day-1"},
            "posts": [{"text": "Scoreboard.", "mentions": [], "kind": "scoreboard"}],
        })
        ...
        # ---- Save content ----
        portfolio_summaries = {
            aid: {"cash": pm.load(aid).cash, "positions": [p.ticker for p in pm.load(aid).positions]}
            for aid in agent_results
        }
        ...
        assert day_before == day_after, f"Day number changed: {day_before} → {day_after}"

        # Call save again (retry scenario)
        step_save_content(
            bundle_date=TRADE_DATE, market_data=market_data, agent_results=agent_results,
            agent_posts=agent_posts, portfolio_summaries=portfolio_summaries,
            leaderboard=leaderboard, blog_draft=blog_draft, oracle_posts=oracle_posts,
        )
        day_after_retry = get_day_number(for_date=TRADE_DATE)
        assert day_after_retry == day_before, f"Day number changed on retry: {day_before} → {day_after_retry}"
        ...
        assert set(loaded["agents"].keys()) == {"satoshi", "steady-eddie-eur", "yolo-sapiens-usd"}
