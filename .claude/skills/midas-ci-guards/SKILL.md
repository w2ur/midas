...
**The session half's ORDINARY outcome is `already_merged`, and that is why it was red 11 runs out of 12 (2026-09-18).** The sandbox's direct `git push origin HEAD:main` normally succeeds, and the sandbox then pushes its own `claude/**` branch anyway — so the commit this workflow's session half classifies is already an ancestor of `origin/main`, and the merge path is the 403/GH006 exception, not the rule. `already_merged` was computed only under `kind == "triggers"`, so for a session branch it was hard-wired `false`, every healthy day reached "Reject stale sessions" with main's newest session date *equal* to the incoming one, and the job exited 1. Run 35270947225 (2026-09-17) is the shape: branch tip `c96ffd291` **is** main's `c96ffd291`, `session-integrity.yml` green on that exact sha at 20:26:51, the auto-merge run created at 20:27:50 and red at `incoming session date: 2026-09-17` / `newest on main: 2026-09-17`. Run 33921355238 (2026-09-04, inside the required-check window) is the one real session merge in the last 14 runs. Skipping the stale rule and the merge on that path loses no coverage: `session-integrity.yml` runs `on: push: branches: [main]` with no path filter and applies ledger-integrity, cash-replay, append-only and the same `check_session_freshness.py` assertion to the sha that landed. **"Verify session-integrity rules" is NOT skipped**, because its artifacts assertion ("a session commit publishes something under `data/posts|blog|output`") is the one session rule `session-integrity.yml` does not hold — its `check` job exits 0 with "nothing to check" on a commit that carries journals and baselines but no posts; gating that step on `already_merged` would have made the rule dark on the ordinary path (found in the first review of this change, by running both scripts against the same artifact-less commit). **Ancestry is the test, not the session date** — a branch carrying a repair main does not have is not an ancestor, so it still goes down the full path and still goes red if it is genuinely stale. The session kind still **reports nothing** (the reporter is gated on `kind == 'triggers'`; `session-watchdog` is the session kind's only consumer). Second cost, same defect: "Delete merged branch" sits AFTER the stale step in the same job, so the job died before the delete ever ran and **15 merged `claude/*` branches accumulated on origin** — every one of them an ancestor of `origin/main`, i.e. a session that landed. Nothing on origin reads `claude/**` (`check_triggers.py` refuses only on `FALLBACK_BRANCH_PREFIX == "triggers/"`, and `session-watchdog` reads main's commit subjects), so they are noise rather than a hazard — but the workflow fix alone cannot clear the existing ones: those branches will never be pushed again, so nothing re-triggers the workflow for them.
...
**The failure-issue tracker has no reader outside GitHub.** The machine-side queue that used to read its open issues live from origin was retired from `~/.claude` on 2026-09-24 (decision D7), so an open failure issue now reaches the owner through GitHub notifications only. That is the shape that once let 17 failure issues go unread for 11 days — a channel whose only off-GitHub reader had to be summoned by hand — so before relying on a failure issue being seen, check what actually reads it today rather than believing a line here.
...
cross-check: a record that is not on main after `before` is a failure. "Landed" is read from `git push --porcelain`, not the exit code: a rebase that empties the commit (the same change already upstream) exits 0 with `[up to date]` and HEAD is another writer's commit, so it records nothing (follow-up review r3, M2). A dispatch that cannot be made files **its own** issue
