...
**A hedge follows a hedge thesis, not Criterion 1.** A BUY of any short or inverse ETF the policy under tax-shaped behavior allows — the 3x shorts 3USS.L and QQQS.L, and the inverse funds DSP5.PA, BX4.PA, XDEB.DE, DXSN.DE — is a hedge. It needs no bull case for the ETF itself: it rests on a coherent bearish thesis in the notes for the index it inverts, sized at 1/L of the exposure it offsets, where L is the fund's leverage: DSP5.PA and XDEB.DE are 1x (full size), BX4.PA and DXSN.DE are 2x (half), 3USS.L and QQQS.L are 3x (a third). **Criterion 3 wins over the sizing**: if the correctly sized hedge falls below the EUR 250 minimum (a third of one EUR 400 position is EUR 133), do not place it — do not round it up to the band, which would over-hedge. Hedge only when the exposure on that index is large enough, and never above the EUR 400 cap. For a hedge, Criterion 2 weighs its cost against the drawdown it protects, not against an expected gain. Criterion 3 applies in full.
