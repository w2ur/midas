...
3. The paper broker (`engine/paper_broker.py::fill_day`) enforces 20 distinct rejection/cancel reason codes (notional cap, order-count cap, universe allowlist, per-agent ticker deny-list, drawdown halt, price lookup, cash/position checks, long-only shares>0, FX-rate, trigger-expiry, agent cancellations, and more) and writes `data/orders/inbox/YYYY-MM-DD.jsonl`.
