...
The orchestrator runs on **Claude Opus 5** (pinned in the RemoteTrigger `job_config`, outside this repo). The 10 traders and the Manager are not pinned: their dispatch **follows the `opus` alias**. `session_costs.dispatches[]` in each day's bundle records, per dispatch, the alias (`model`) and `model_id`, which holds what `ANTHROPIC_DEFAULT_OPUS_MODEL` pins in the session's environment and is `null` otherwise — expect `null` in the cloud, since the routine pins nothing. The release a dispatch actually ran on is in the session transcript (each subagent message names its model) and is **not yet captured** into the record; a `null` means unknown, not Opus 5. The alias moved to Opus 5.5 on 2026-09-23, inside the Gate C window, and is kept deliberately ([METHODOLOGY](../../METHODOLOGY.md#opus-alias-2026-09-23)). The Oracle follows `sonnet`.
