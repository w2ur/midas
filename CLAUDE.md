...
- **Partial bars are revised, not frozen.** `append_new_rows` is append-only and keeps only unseen dates. That is correct for equities and FX, whose bars are final by fetch time, but wrong for 24/7 crypto: a bar written at 23:44 UTC is still forming. `engine.ohlcv_ingest.merge_rows(path, df, revise_from=...)` replaces an already-stored row on or after `revise_from`, and `scripts/fetch_ohlcv.py` requests a 1-day window for crypto symbols only. **Expect crypto rows for the current day to change value on the following run — that is the mechanism working.** A store containing an unparseable line degrades to append-only rather than being reordered.
...
- **Manual runs before market close can freeze a partial equity bar.** Equities run with `revise_days=0` — only crypto gets the 1-day revision window, so an equity bar written before the close can never be corrected. The scheduled crons are safe (the weekday cron runs 22:30 UTC, after the close; the weekend cron is crypto-only), but a `workflow_dispatch` or a local `fetch_ohlcv.py` run at midday now writes real, same-day rows instead of being skipped — and if the US market hasn't closed yet, those equity rows are partial and permanent. Only run this ad hoc after the close, or accept the freeze.
