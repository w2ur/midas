...
## Repo Split (SP4 — midas-core mirror)
...
- **Manifest** (in `scripts/sync_core.py`): core gets `engine/`, the 10 reusable scripts, `examples/demo-desk/`, generic data (`data/strategies`, `data/universes`, ticker maps), and 44 hermetic engine test files. Live-only: the 5 data/publish scripts + 27 tests that either import a live-only script, read the committed OHLCV store, or hardcode the live cast (roster/personas/schedule/capital). Core ships a core-native root `roster.yaml` (demo cast) so `get_config()` and the `midas_data_root` conftest fixture work at its root.
- **Deferred to SP5:** the package-dependency cutover (delete `engine/` from live, install core as a package), going public (LICENSE/DISCLAIMER), and reclaiming the reclassified engine tests' coverage in core via `skipif` markers. `sync_core.apply` does not yet prune stale files.
